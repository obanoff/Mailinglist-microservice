...
type MailServer struct {
	// embedded type based on proto file's service
	pb.UnimplementedMailingListServiceServer
	db *sql.DB
}

// convert proto buffer message into mdb data structure
...
// reverse converting
func mdbEntryToPbEntry(mdb *mdb.EmailEntry) pb.EmailEntry {
	t := mdb.ConfirmedAt.Unix()
...
// pb.UnimplementedMailingListSevriceServer implementation

func emailResponce(db *sql.DB, email string) (*pb.EmailResponse, error) {
...
func (s *MailServer) GetEmail(ctx context.Context, req *pb.GetEmailRequest) (*pb.EmailResponse, error) {
	...
	return emailResponce(s.db, req.EmailAddr)
}
...
func (s *MailServer) GetEmailBatch(ctx context.Context, req *pb.GetEmailBatchRequest) (*pb.GetEmailBatchResponse, error) {
	...
	res := make([]*pb.EmailEntry, 0, len(entries))
	for _, entry := range entries {
		conv := mdbEntryToPbEntry(&entry)
		res = append(res, &conv)
	}

	return &pb.GetEmailBatchResponse{EmailEntries: res}, nil
}
...
func (s *MailServer) UpdateEmail(ctx context.Context, req *pb.UpdateEmailRequest) (*pb.EmailResponse, error) {
	...
	return emailResponce(s.db, entry.Email)
}
