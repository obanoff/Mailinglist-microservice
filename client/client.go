...
func createEmail(client pb.MailingListServiceClient, addr string) *pb.EmailEntry {
	...
	// terminate request if it takes longer than timeout
	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	// terminate request if function completed in less than timeout
	defer cancel()

	res, err := client.CreateEmail(ctx, &pb.CreateEmailRequest{
		EmailAddr: addr,
	})
...
func getEmail(client pb.MailingListServiceClient, addr string) *pb.EmailEntry {
	...
	res, err := client.GetEmail(ctx, &pb.GetEmailRequest{
		EmailAddr: addr,
	})
...
func updateEmail(client pb.MailingListServiceClient, entry *pb.EmailEntry) *pb.EmailEntry {
	...
	res, err := client.UpdateEmail(ctx, &pb.UpdateEmailRequest{
		EmailEntry: entry,
	})
...
func deleteEmail(client pb.MailingListServiceClient, addr string) *pb.EmailEntry {
	...
	res, err := client.DeleteEmail(ctx, &pb.DeleteEmailRequest{
		EmailAddr: addr,
	})
